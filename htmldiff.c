  ...
  while (isspace (side->character) || side->character == '<')
	...
	case COPY_DELETED:
	  if (side->character == '\n')
	    ...
	    if (side->character != EOF) putc (side->character, output_file);
	  }
	  break;
...
static void
copy_until_ordinal (SIDE *side, int ordinal, int mark_as)
{
  while (side->position < ordinal)
    {
      if (mark_as != 'd')
	...
	putc(' ', output_file);
      ...
      switch (mark_as) {
	case 'i': start_of_insert (); break;
	case 'd': start_of_delete (); break;
      }
      copy_word (side, output_file);
      switch (mark_as) {
	case 'i': end_of_insert (); break;
	case 'd': end_of_delete (); break;
      }
    }
}
